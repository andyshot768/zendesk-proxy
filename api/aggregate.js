...
// ── fetch a time slice with full pagination ──────────────────────────────────
async function fetchSlice(subdomain, auth, startDT, endDT, agg) {
  // Zendesk search supports datetime: created>=2026-02-01T00:00:00Z
  const q = encodeURIComponent(`type:ticket created>=${startDT} created<${endDT}`);
  let page = 1;
  let total = 0;
  let hasMore = true;
  while (hasMore) {
    ...
    if (status !== 200) throw new Error(`Zendesk ${status}: ${JSON.stringify(data).slice(0,200)}`);
    const batch = data.results || [];
    batch.forEach(t => processTicket(agg, t));
    total += batch.length;
    // Zendesk hard cap: stop at page 10 (1000 results) and sub-divide if needed
...
// ── fetch one day — auto-subdivides into 6h blocks if day > 900 tickets ──────
async function fetchDay(subdomain, auth, day, agg) {
  // First try the whole day
  const startDT = `${day}T00:00:00Z`;
  const endDT   = `${day}T23:59:59Z`;
  const q = encodeURIComponent(`type:ticket created>=${startDT} created<=${endDT}`);

  // Check count first
  const { status: cs, data: cd } = await zdRequest(
    subdomain, auth,
    `/api/v2/search/count.json?query=${q}`
  );
  const count = (cs === 200 ? cd.count : 0) || 0;

  if (count <= 900) {
    // Safe to fetch in one go
    await fetchSlice(subdomain, auth, startDT, endDT + ' ', agg);
    return;
  }

  // Day has >900 — split into 6-hour blocks
  const blocks = [
    [`${day}T00:00:00Z`, `${day}T05:59:59Z`],
    [`${day}T06:00:00Z`, `${day}T11:59:59Z`],
    [`${day}T12:00:00Z`, `${day}T17:59:59Z`],
    [`${day}T18:00:00Z`, `${day}T23:59:59Z`],
  ];

  for (const [s, e] of blocks) {
    // Check block count
    const bq = encodeURIComponent(`type:ticket created>=${s} created<=${e}`);
    const { status: bs, data: bd } = await zdRequest(subdomain, auth, `/api/v2/search/count.json?query=${bq}`);
    const bcount = (bs === 200 ? bd.count : 0) || 0;

    if (bcount <= 900) {
      await fetchSlice(subdomain, auth, s, e + ' ', agg);
    } else {
      // Split into 1-hour blocks
      const startH = parseInt(s.slice(11, 13));
      for (let h = startH; h < startH + 6; h++) {
        const hh = String(h).padStart(2, '0');
        const hs = `${day}T${hh}:00:00Z`;
        const he = `${day}T${hh}:59:59Z`;
        await fetchSlice(subdomain, auth, hs, he + ' ', agg);
      }
    }
  }
}
